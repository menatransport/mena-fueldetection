...
export const Graph = () => {
    ...
    const { selectedGroup, navigateToPrevious, navigateToNext, allGroups, currentIndex } = useFuelData();
    const defaultImageUrl = '@/public/opengraph.jpg';
                ...
                >
                    <div className="relative w-full h-full flex items-center justify-center">
                        ...
                        {!isImageLoaded && (
                            <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-90 rounded-lg">
                                <div className="text-center">
                                    ...
                                    <p className="text-sm text-gray-400">{selectedGroup?.id || 'Default'}</p>
                                </div>
                            </div>
                        )}
                        ...
                        <div 
                                ...
                                title={`ไปซ้าย ${allGroups.length > 0 ? `(${currentIndex + 1}/${allGroups.length})` : ''}`}
                            >
                                <svg className="w-14 h-14 text-white hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                             ...
                             <div 
                                ...
                                onClick={handleNext}
                                title={`ไปขวา ${allGroups.length > 0 ? `(${currentIndex + 1}/${allGroups.length})` : ''}`}
                            >
                                <svg className="w-14 h-14 text-white hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
